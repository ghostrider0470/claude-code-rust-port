...
- `transcript-find <selector> <query>` with optional `--limit <n>` (inspect-only command that resolves a single-session selector — raw `session_id`, `latest`, or `label:<name>` — through the shared selector-resolution path and searches only the resolved session's persisted transcript for entries whose prompt text contains `<query>` case-insensitively as a substring, mirroring `session-find` query semantics but scoped to a single selected session; output is a deterministic `{ selector, resolved_session_id, query, created_at_ms, updated_at_ms, total_entries, match_count, matches }` JSON shape where `selector` and `query` echo the raw input, `resolved_session_id` is the persisted id the selector actually maps to, `total_entries` is the full transcript length, `match_count == matches.len()`, and each entry in `matches` carries at least `turn_index` and `prompt` in the source transcript's `turn_index` order; an empty query and a no-match query both succeed cleanly with `match_count == 0` and an empty `matches` array; omitting `--limit` returns every match unchanged, `--limit <n>` truncates `matches` to at most the first `n` entries in the same source `turn_index` ordering and recomputes `match_count` so `match_count == matches.len()` still holds, `--limit 0` returns `match_count: 0` with an empty `matches` array cleanly, a `--limit` larger than the match count returns every match cleanly, and the outer JSON shape is preserved — the limited form is a truncation of the existing `matches` array rather than a new wrapper object; negative and non-numeric `--limit` values fail cleanly at parse time; preserves existing selector failure semantics unchanged — unknown ids/labels → `SessionNotFound`, duplicate labels → `AmbiguousLabel`, empty `label:` → `MalformedSelector`; does not mutate any persisted session, transcript entry, label, pinned flag, id, path, or ordering metadata)
