...
- `transcript-show <selector>` with optional `--limit <n>` (accepts raw `session_id`, `latest`, or `label:<name>` — routed through the shared selector-resolution path; machine-readable JSON output restates the owning `session_id` and preserves turn ordering; omitting `--limit` returns every persisted transcript entry in the existing `{ session_id, created_at_ms, updated_at_ms, entries }` JSON shape unchanged, `--limit <n>` truncates `entries` to at most the first `n` entries in the same canonical append order while the outer JSON shape is preserved — the limited form is a truncation of the existing `entries` array rather than a new wrapper object, `--limit 0` returns `entries: []` cleanly, a `--limit` greater than the persisted transcript length returns every entry cleanly, and negative / non-numeric `--limit` values fail cleanly at parse time; selector failures stay deterministic — unknown id/label → `SessionNotFound`, duplicate labels → `AmbiguousLabel`, empty `label:` → `MalformedSelector`; does not mutate any persisted session, transcript entry, label, pinned flag, id, path, or ordering metadata)
