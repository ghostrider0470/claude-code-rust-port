...
- `session-show <selector>` with optional `--limit <n>` (accepts raw `session_id`, `latest`, or `label:<name>` — routed through the shared selector-resolution path; machine-readable JSON output surfaces the actual resolved `session_id` rather than the typed selector; omitting `--limit` returns the persisted session unchanged in its existing JSON shape, `--limit <n>` truncates `messages` to at most the first `n` entries in the same canonical append order while the outer JSON shape is preserved — the limited form is a truncation of the existing `messages` array rather than a new wrapper object, `--limit 0` returns `messages: []` cleanly, a `--limit` greater than the persisted message count returns every message cleanly, and negative / non-numeric `--limit` values fail cleanly at parse time; selector failures stay deterministic — unknown id/label → `SessionNotFound`, duplicate labels → `AmbiguousLabel`, empty `label:` → `MalformedSelector`; does not mutate any persisted session, transcript entry, label, pinned flag, id, path, or ordering metadata)
