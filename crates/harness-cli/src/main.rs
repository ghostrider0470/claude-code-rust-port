...
enum CliCommand {
    ...
    TranscriptSmallestGap {
        selector: String,
    },
    TranscriptGapCount {
        selector: String,
    },
    TranscriptMissingTurnCount {
        selector: String,
    },
}
...
fn render_command(engine: &RuntimeEngine, command: CliCommand) -> String {
    match command {
        ...
        CliCommand::SessionPins { limit, tail } => {
            let mut pins = engine
                .list_session_pins()
        ...
        CliCommand::TranscriptMissingTurnIndexes {
            ...
            if let Some(limit) = limit {
                ...
                let total = missing.missing_turn_indexes.len();
                let skip = total.saturating_sub(tail);
                missing.missing_turn_indexes.drain(..skip);
            }
        ...
        CliCommand::TranscriptGapRanges {
            ...
            if let Some(limit) = limit {
                ...
                let total = gap_ranges.gap_ranges.len();
                let skip = total.saturating_sub(tail);
                gap_ranges.gap_ranges.drain(..skip);
            }
            serde_json::to_string_pretty(&gap_ranges)
                .expect("serialize transcript gap-ranges")
        }
        CliCommand::TranscriptLargestGap { selector } => {
            let largest_gap = engine
                .largest_gap_session_transcript(&selector)
                .expect("largest-gap persisted session transcript");
            serde_json::to_string_pretty(&largest_gap)
                .expect("serialize transcript largest-gap")
        }
        CliCommand::TranscriptSmallestGap { selector } => {
            let smallest_gap = engine
                .smallest_gap_session_transcript(&selector)
                .expect("smallest-gap persisted session transcript");
            serde_json::to_string_pretty(&smallest_gap)
                .expect("serialize transcript smallest-gap")
        }
        CliCommand::TranscriptGapCount { selector } => {
            let gap_count = engine
                .gap_count_session_transcript(&selector)
                .expect("gap-count persisted session transcript");
            serde_json::to_string_pretty(&gap_count)
                .expect("serialize transcript gap-count")
        }
        CliCommand::TranscriptMissingTurnCount { selector } => {
            let missing_turn_count = engine
                .missing_turn_count_session_transcript(&selector)
                .expect("missing-turn-count persisted session transcript");
            serde_json::to_string_pretty(&missing_turn_count)
                .expect("serialize transcript missing-turn-count")
        }
    }
}
...
mod tests {
    ...
    use harness_session::{
        ...
        SessionTranscriptGapCount, SessionTranscriptGapRange, SessionTranscriptGapRanges,
        SessionTranscriptHasEntries, SessionTranscriptHasTurnGaps, SessionTranscriptLargestGap,
        SessionTranscriptLargestGapRun, SessionTranscriptSmallestGap,
        SessionTranscriptSmallestGapRun,
        SessionTranscriptLastTurn, SessionTranscriptMissingTurnCount,
        SessionTranscriptMissingTurnIndexes, SessionTranscriptRange,
    ...
    fn session_find_tail_ids(
        ...
        let parsed: Vec<SessionFindResult> =
            serde_json::from_str(&output).expect("parse session-find tail output");
        parsed
            .into_iter()
            .map(|entry| entry.session_id.to_string())
            .collect()
    ...
    #[test]
    fn session_find_omitted_tail_preserves_unlimited_behavior_exactly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
    ...
    #[test]
    fn session_find_tail_zero_returns_empty_array_without_mutating_store() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        // Unlimited search still returns every match afterwards.
        assert_eq!(session_find_ids(&engine, "review", None).len(), 2);
    ...
    #[test]
    fn session_find_tail_one_returns_only_last_row_of_newest_first_ordering() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let unlimited = session_find_ids(&engine, "review", None);
        assert_eq!(unlimited, vec![newer.clone(), older.clone()]);

        // `session-find` is already newest-first, so the tail of the ordered
        // result set is the oldest matching session.
        let tailed = session_find_tail_ids(&engine, "review", Some(1));
        assert_eq!(tailed, vec![older]);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn session_find_tail_smaller_than_match_count_returns_suffix_of_default_ordering() {
        let root = temp_session_root();
        let engine = temp_engine(&root);

        // Three matching sessions plus one non-matching middle session to
        // prove the tail slices the matched listing, not a raw ordering.
        let a = bootstrap_session_id(&engine, "review first");
        std::thread::sleep(std::time::Duration::from_millis(2));
        let non_match = bootstrap_session_id(&engine, "unrelated middle");
        std::thread::sleep(std::time::Duration::from_millis(2));
        ...
        let tailed = session_find_tail_ids(&engine, "review", Some(2));
        assert_eq!(tailed, unlimited[unlimited.len() - 2..].to_vec());
        assert!(
            !tailed.iter().any(|id| id == &non_match),
            "non-matching session must stay omitted under tailing"
        );
    ...
    #[test]
    fn session_find_tail_exceeding_match_count_returns_all_matching_sessions_unchanged() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let tailed = session_find_tail_ids(&engine, "review", Some(99));
    ...
    #[test]
    fn session_find_tail_empty_or_no_match_query_returns_empty_array_regardless_of_tail() {
        let root = temp_session_root();
        let engine = temp_engine(&root);

        let _ = bootstrap_session_id(&engine, "review bash");

        for tail in [None, Some(0), Some(1), Some(99)] {
            let empty_query = render_command(
                &engine,
                CliCommand::SessionFind {
                    query: String::new(),
                    limit: None,
    ...
    #[test]
    fn session_find_tail_preserves_per_row_matches_arrays_unchanged() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let tailed_output = render_command(
            &engine,
            CliCommand::SessionFind {
                query: "review".to_string(),
                limit: None,
        ...
        let tailed: Vec<SessionFindResult> =
            serde_json::from_str(&tailed_output).expect("parse tailed find output");
        assert_eq!(tailed.len(), 1);

        // The row retained under `--tail 1` is the *oldest* matching session
        // (the last element of the newest-first ordered result set) and its
        // `matches` array is unchanged — tailing only slices, it does not
        // mutate rows.
        assert_eq!(tailed[0], unlimited[1]);
        assert_eq!(tailed[0].matches, unlimited[1].matches);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn session_find_tail_does_not_mutate_persisted_store() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        for tail in [Some(0), Some(1), Some(99)] {
            let _ = render_command(
                &engine,
                CliCommand::SessionFind {
                    query: "review".to_string(),
    ...
    #[test]
    fn session_find_invalid_tail_is_rejected_by_clap_parse() {
        use clap::Parser;

        let err = Cli::try_parse_from(["harness", "session-find", "review", "--tail", "-1"])
    ...
    #[test]
    fn session_find_limit_and_tail_are_mutually_exclusive_at_parse_time() {
        use clap::Parser;
    ...
    fn label_session(engine: &RuntimeEngine, id: &str, label: &str) {
        let _ = render_command(
            engine,
            CliCommand::SessionRename {
    ...
    #[test]
    fn session_labels_omitted_limit_preserves_current_unlimited_behavior_exactly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
    ...
    #[test]
    fn session_labels_with_limit_zero_returns_empty_array_cleanly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        assert_eq!(labeled_session_ids(&engine, None), vec![id]);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn session_labels_with_limit_one_returns_only_newest_preserving_ordering() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let limited = labeled_session_ids(&engine, Some(1));
    ...
    #[test]
    fn session_labels_with_limit_smaller_than_labeled_subset_returns_prefix_of_default_ordering() {
        let root = temp_session_root();
        let engine = temp_engine(&root);

        // Three labeled sessions plus one unlabeled middle to prove the
        // limit slices the labeled listing, not the raw sessions listing.
        let a = bootstrap_session_id(&engine, "first");
        std::thread::sleep(std::time::Duration::from_millis(2));
        ...
        let limited = labeled_session_ids(&engine, Some(2));
        assert_eq!(limited, unlimited[..2].to_vec());
        assert!(
            !limited.iter().any(|id| id == &unlabeled),
            "unlabeled session must stay omitted under limiting"
        );
    ...
    #[test]
    fn session_labels_with_limit_exceeding_labeled_subset_returns_all_labeled_sessions() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let limited = labeled_session_ids(&engine, Some(99));
        assert_eq!(limited, unlimited);
    ...
    #[test]
    fn session_labels_with_limit_keeps_duplicate_labels_visible_as_separate_rows() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        // A tighter limit still slices deterministically from the newest-first
        // ordering — the duplicate label survives the slice as a single row
        ...
        assert_eq!(limited_one_entries[0].session_id.to_string(), second_id);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn session_labels_limit_does_not_mutate_persisted_store() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
    ...
    #[test]
    fn session_labels_invalid_limit_is_rejected_by_clap_parse() {
        use clap::Parser;

        let err = Cli::try_parse_from(["harness", "session-labels", "--limit", "-1"])
            .expect_err("negative --limit must fail at parse time");
        let rendered = err.to_string();
        assert!(
            rendered.contains("--limit") || rendered.contains("-1"),
        ...
        let err = Cli::try_parse_from(["harness", "session-labels", "--limit", "not-a-number"])
            .expect_err("non-numeric --limit must fail at parse time");
        let rendered = err.to_string();
        assert!(
            rendered.contains("--limit") || rendered.contains("not-a-number"),
    ...
    fn session_pins_orders_newest_first_omits_unpinned_and_surfaces_optional_label() {
        ...
        let pins_output = render_command(&engine, CliCommand::SessionPins { limit: None, tail: None });
        ...
        let pins_output = render_command(&readme_engine, CliCommand::SessionPins { limit: None, tail: None });
    ...
    fn session_pins_empty_store_matches_readme_empty_example() {
        ...
        // Truly empty store: no persisted sessions at all.
        let output = render_command(&engine, CliCommand::SessionPins { limit: None, tail: None });
        assert_eq!(
            output,
    ...
    fn pinned_session_ids(engine: &RuntimeEngine, limit: Option<usize>) -> Vec<String> {
        let output = render_command(engine, CliCommand::SessionPins { limit, tail: None });
        let parsed: serde_json::Value =
    ...
    fn session_pins_omitted_limit_preserves_current_unlimited_behavior_exactly() {
        ...
        let baseline = render_command(&engine, CliCommand::SessionPins { limit: None, tail: None });
        let explicit_large = render_command(
            &engine,
    ...
    fn session_pins_with_limit_zero_returns_empty_array_cleanly() {
        ...
        let output = render_command(&engine, CliCommand::SessionPins { limit: Some(0), tail: None });
        assert_eq!(output, "[]");
    ...
    fn session_pins_with_limit_keeps_labeled_pinned_rows_label_field_visible() {
        ...
        let output = render_command(&engine, CliCommand::SessionPins { limit: Some(2), tail: None });
        let entries: Vec<SessionPinEntry> =
    ...
    fn session_pins_limit_does_not_mutate_persisted_store() {
        ...
        let before_ta = engine.load_transcript(&a).expect("transcript a before");
        let before_tb = engine.load_transcript(&b).expect("transcript b before");
        let before_pins = render_command(&engine, CliCommand::SessionPins { limit: None, tail: None });

        let _ = render_command(&engine, CliCommand::SessionPins { limit: Some(0), tail: None });
        let _ = render_command(&engine, CliCommand::SessionPins { limit: Some(1), tail: None });
        let _ = render_command(&engine, CliCommand::SessionPins { limit: Some(99), tail: None });
        ...
        let after_pins = render_command(&engine, CliCommand::SessionPins { limit: None, tail: None });
    ...
    fn pinned_session_tail_ids(engine: &RuntimeEngine, tail: Option<usize>) -> Vec<String> {
        let output = render_command(engine, CliCommand::SessionPins { limit: None, tail });
        let parsed: serde_json::Value =
            serde_json::from_str(&output).expect("parse tailed session-pins output");
        parsed
            .as_array()
            .expect("session-pins output is a JSON array")
            .iter()
    ...
    #[test]
    fn session_pins_omitted_tail_preserves_current_unlimited_behavior_exactly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let baseline = render_command(&engine, CliCommand::SessionPins { limit: None, tail: None });
        let explicit_none = render_command(&engine, CliCommand::SessionPins { limit: None, tail: None });
        assert_eq!(baseline, explicit_none);
        assert!(baseline.trim_start().starts_with('['));
    ...
    #[test]
    fn session_pins_tail_zero_returns_empty_array_without_mutating_store() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let output = render_command(&engine, CliCommand::SessionPins { limit: None, tail: Some(0) });
        assert_eq!(output, "[]");
        assert_eq!(
            pinned_session_ids(&engine, None),
            vec![newer_id.clone(), older_id.clone()]
        );
        assert_eq!(engine.load_session(&older_id).expect("load older after"), before_older);
        assert_eq!(engine.load_session(&newer_id).expect("load newer after"), before_newer);
    ...
    #[test]
    fn session_pins_tail_one_returns_only_oldest_retained_row_preserving_ordering() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let tailed = pinned_session_tail_ids(&engine, Some(1));
        assert_eq!(tailed, vec![older_id]);
    ...
    #[test]
    fn session_pins_tail_smaller_than_pinned_subset_returns_suffix_of_default_ordering() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let tailed = pinned_session_tail_ids(&engine, Some(2));
        assert_eq!(tailed, unlimited[unlimited.len() - 2..].to_vec());
        assert!(
            !tailed.iter().any(|id| id == &unpinned),
            "unpinned session must stay omitted under tailing"
        );
    ...
    #[test]
    fn session_pins_tail_exceeding_pinned_subset_returns_all_pinned_sessions() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let tailed = pinned_session_tail_ids(&engine, Some(99));
        assert_eq!(tailed, unlimited);
    ...
    #[test]
    fn session_pins_invalid_tail_is_rejected_by_clap_parse() {
        use clap::Parser;

        let err = Cli::try_parse_from(["harness", "session-pins", "--tail", "-1"])
            .expect_err("negative --tail must fail at parse time");
        let rendered = err.to_string();
        assert!(
            rendered.contains("--tail") || rendered.contains("-1"),
        ...
        let err = Cli::try_parse_from(["harness", "session-pins", "--tail", "not-a-number"])
            .expect_err("non-numeric --tail must fail at parse time");
        let rendered = err.to_string();
        assert!(
            rendered.contains("--tail") || rendered.contains("not-a-number"),
    ...
    #[test]
    fn session_pins_limit_and_tail_are_mutually_exclusive_at_parse_time() {
        use clap::Parser;
    ...
    fn transcript_turn_indexes_output(
        engine: &RuntimeEngine,
        selector: &str,
        limit: Option<usize>,
    ...
    #[test]
    fn transcript_turn_indexes_omitted_limit_preserves_current_unlimited_behavior_exactly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        assert!(
            parsed.is_object(),
            "transcript-turn-indexes must return an object, got: {baseline}"
        );
        for key in [
            "selector",
            "resolved_session_id",
            ...
            assert!(
                ...
                "expected key {key} in transcript-turn-indexes output, got: {baseline}"
            );
        }
    ...
    #[test]
    fn transcript_turn_indexes_with_limit_zero_returns_empty_turn_indexes_but_full_total_entries() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let turn_indexes = transcript_turn_indexes_output(&engine, &id, Some(0));
        assert_eq!(turn_indexes.total_entries, 3);
        assert!(turn_indexes.turn_indexes.is_empty());

        let after_transcript = engine.load_transcript(&id).expect("reload after turn-indexes");
        assert_eq!(after_transcript, before_transcript);
    ...
    #[test]
    fn transcript_turn_indexes_with_limit_one_returns_only_first_index_in_ascending_order() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let turn_indexes = transcript_turn_indexes_output(&engine, &id, Some(1));
        assert_eq!(turn_indexes.total_entries, 4);
        assert_eq!(turn_indexes.turn_indexes, vec![0]);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn transcript_turn_indexes_with_limit_smaller_than_total_truncates_in_ascending_order() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let turn_indexes = transcript_turn_indexes_output(&engine, &id, Some(2));
        assert_eq!(turn_indexes.total_entries, 4);
        assert_eq!(turn_indexes.turn_indexes, vec![0, 1]);
    ...
    #[test]
    fn transcript_turn_indexes_with_limit_larger_than_total_returns_all_indexes() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let turn_indexes = transcript_turn_indexes_output(&engine, &id, Some(99));
        assert_eq!(turn_indexes.total_entries, 3);
        assert_eq!(turn_indexes.turn_indexes, vec![0, 1, 2]);
    ...
    #[test]
    fn transcript_turn_indexes_empty_transcript_with_and_without_limit_returns_empty_array() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        for limit in [None, Some(0), Some(5)] {
            let turn_indexes = transcript_turn_indexes_output(&engine, &session_id, limit);
            assert_eq!(turn_indexes.total_entries, 0);
            assert!(turn_indexes.turn_indexes.is_empty());
        }
    ...
    #[test]
    fn transcript_turn_indexes_latest_selector_honors_limit() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let turn_indexes = transcript_turn_indexes_output(&engine, "latest", Some(2));
        assert_eq!(turn_indexes.selector, "latest");
        assert_eq!(turn_indexes.resolved_session_id.to_string(), newer);
        assert_eq!(turn_indexes.total_entries, 3);
        assert_eq!(turn_indexes.turn_indexes, vec![0, 1]);
    ...
    #[test]
    fn transcript_turn_indexes_label_selector_honors_limit() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let turn_indexes =
            transcript_turn_indexes_output(&engine, "label:runtime-review", Some(1));
        assert_eq!(turn_indexes.selector, "label:runtime-review");
        assert_eq!(turn_indexes.resolved_session_id.to_string(), id);
        assert_eq!(turn_indexes.total_entries, 3);
        assert_eq!(turn_indexes.turn_indexes, vec![0]);
    ...
    #[test]
    fn transcript_turn_indexes_limit_does_not_mutate_persisted_transcript_or_metadata() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let _ = transcript_turn_indexes_output(&engine, &id, Some(0));
        let _ = transcript_turn_indexes_output(&engine, &id, Some(1));
        let _ = transcript_turn_indexes_output(&engine, &id, Some(99));
    ...
    #[test]
    fn transcript_turn_indexes_invalid_limit_is_rejected_by_clap_parse() {
        use clap::Parser;
    ...
    fn transcript_missing_turn_indexes_tail_output(
        engine: &RuntimeEngine,
        selector: &str,
        tail: Option<usize>,
    ) -> SessionTranscriptMissingTurnIndexes {
        ...
        serde_json::from_str(&output).expect("parse transcript-missing-turn-indexes tail output")
    }
    ...
    #[test]
    fn transcript_missing_turn_indexes_omitted_tail_preserves_unlimited_behavior_exactly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let parsed: SessionTranscriptMissingTurnIndexes = serde_json::from_str(&baseline)
            .expect("parse transcript-missing-turn-indexes baseline");
        assert_eq!(parsed.total_entries, 3);
        assert_eq!(parsed.missing_turn_indexes, vec![2, 3, 5, 6]);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn transcript_missing_turn_indexes_tail_zero_returns_empty_missing_but_full_total_entries() {
        let root = temp_session_root();
        let engine = temp_engine(&root);

        let session_id = seed_multi_gap_transcript(&engine);
        let before_session = engine.load_session(&session_id).expect("reload session");
        let before_transcript = engine
            .load_transcript(&session_id)
            .expect("reload transcript");

        let missing = transcript_missing_turn_indexes_tail_output(&engine, &session_id, Some(0));
        assert_eq!(
            missing.total_entries, 3,
            "--tail 0 must keep total_entries at the persisted transcript length"
        );
        assert!(
            missing.missing_turn_indexes.is_empty(),
            "--tail 0 must return an empty missing_turn_indexes array"
        );
        ...
        let after_transcript = engine
            .load_transcript(&session_id)
            .expect("reload transcript after missing-turn-indexes");
        assert_eq!(
            after_session, before_session,
            "--tail 0 must not mutate the persisted session state"
        );
    ...
    #[test]
    fn transcript_missing_turn_indexes_tail_returns_newest_n_indexes_in_ascending_order() {
        let root = temp_session_root();
        let engine = temp_engine(&root);

        let session_id = seed_multi_gap_transcript(&engine);

        let missing = transcript_missing_turn_indexes_tail_output(&engine, &session_id, Some(2));
        assert_eq!(missing.resolved_session_id.to_string(), session_id);
        assert_eq!(missing.total_entries, 3);
        assert_eq!(
            missing.missing_turn_indexes,
            vec![5, 6],
            "--tail <n> must return only the newest n missing indexes, preserving ascending order"
        );
    ...
    #[test]
    fn transcript_missing_turn_indexes_tail_larger_than_total_returns_all_missing_indexes_unchanged()
    {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let missing = transcript_missing_turn_indexes_tail_output(&engine, &session_id, Some(99));
        assert_eq!(missing.total_entries, 3);
        assert_eq!(
            missing.missing_turn_indexes,
            vec![2, 3, 5, 6],
            "oversized --tail must return every missing index cleanly"
        );
    ...
    #[test]
    fn transcript_missing_turn_indexes_tail_empty_and_contiguous_transcripts_return_empty_cleanly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);

        let mut session = SessionState::default();
        session.messages.clear();
        let empty_id = session.session_id.to_string();
        engine.store.save(&session).expect("persist empty session");
        let empty_transcript = TranscriptRecord {
            session_id: session.session_id.clone(),
            created_at_ms: session.created_at_ms,
        ...
        for tail in [None, Some(0), Some(5)] {
            let missing = transcript_missing_turn_indexes_tail_output(&engine, &empty_id, tail);
            assert_eq!(missing.total_entries, 0);
            assert!(missing.missing_turn_indexes.is_empty());
        }
        ...
        let contiguous_id = bootstrap_session_id(&engine, "first prompt");
        extend_transcript(&engine, &contiguous_id, &["second prompt", "third prompt"]);
        for tail in [None, Some(0), Some(2), Some(99)] {
            let missing =
                transcript_missing_turn_indexes_tail_output(&engine, &contiguous_id, tail);
            assert_eq!(missing.total_entries, 3);
            assert!(missing.missing_turn_indexes.is_empty());
        }
    ...
    #[test]
    fn transcript_missing_turn_indexes_tail_latest_selector_resolves_to_newest_persisted_session() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let gap_transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(4),
                    prompt: Prompt::new("newer turn four - gaps at 2, 3"),
                },
            ],
        };
        engine
        ...
        let missing = transcript_missing_turn_indexes_tail_output(&engine, "latest", Some(1));
        assert_eq!(missing.selector, "latest");
        assert_eq!(missing.resolved_session_id.to_string(), newer);
        assert_eq!(missing.total_entries, 2);
        assert_eq!(
            missing.missing_turn_indexes,
            vec![3],
            "--tail must surface the newest missing index of the resolved latest session"
        );
    ...
    #[test]
    fn transcript_missing_turn_indexes_tail_label_selector_surfaces_resolved_id_and_does_not_mutate_store(
    ) {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        engine
            .rename_session(&session_id, "runtime-review")
            .expect("attach label for missing-turn-indexes --tail");

        let missing = transcript_missing_turn_indexes_tail_output(
            &engine,
            "label:runtime-review",
            Some(2),
        );
        ...
        assert_eq!(missing.resolved_session_id.to_string(), session_id);
        assert_eq!(missing.total_entries, 3);
        assert_eq!(missing.missing_turn_indexes, vec![5, 6]);

        let reloaded = engine
            .load_session(&session_id)
            .expect("reload after missing-turn-indexes --tail");
        assert_eq!(reloaded.label.as_deref(), Some("runtime-review"));
    ...
    #[test]
    fn transcript_missing_turn_indexes_tail_does_not_mutate_persisted_transcript_or_metadata() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let _ = transcript_missing_turn_indexes_tail_output(&engine, &session_id, Some(0));
        let _ = transcript_missing_turn_indexes_tail_output(&engine, &session_id, Some(1));
        let _ = transcript_missing_turn_indexes_tail_output(&engine, &session_id, Some(99));
    ...
    #[test]
    fn transcript_missing_turn_indexes_invalid_tail_is_rejected_by_clap_parse() {
        use clap::Parser;
    ...
    #[test]
    fn transcript_missing_turn_indexes_limit_and_tail_are_mutually_exclusive_at_parse_time() {
        use clap::Parser;
    ...
    fn transcript_gap_ranges_tail_output(
        engine: &RuntimeEngine,
        selector: &str,
        tail: Option<usize>,
    ) -> SessionTranscriptGapRanges {
        ...
        serde_json::from_str(&output).expect("parse transcript-gap-ranges tail output")
    }
    ...
    #[test]
    fn transcript_gap_ranges_omitted_tail_preserves_unlimited_behavior_exactly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let parsed: SessionTranscriptGapRanges =
            serde_json::from_str(&baseline).expect("parse transcript-gap-ranges baseline");
    ...
    #[test]
    fn transcript_gap_ranges_tail_zero_returns_empty_ranges_but_full_total_entries() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let gap_ranges = transcript_gap_ranges_tail_output(&engine, &session_id, Some(0));
        assert_eq!(
            gap_ranges.total_entries, 5,
            "--tail 0 must keep total_entries at the persisted transcript length"
        );
        assert!(
            gap_ranges.gap_ranges.is_empty(),
            "--tail 0 must return an empty gap_ranges array"
        );
        ...
        let after_transcript = engine
            .load_transcript(&session_id)
            .expect("reload transcript after gap-ranges");
        assert_eq!(
            after_session, before_session,
            "--tail 0 must not mutate the persisted session state"
        );
    ...
    #[test]
    fn transcript_gap_ranges_tail_returns_newest_n_ranges_in_ascending_order() {
        let root = temp_session_root();
        let engine = temp_engine(&root);

        let session_id = seed_triple_gap_transcript(&engine);

        let gap_ranges = transcript_gap_ranges_tail_output(&engine, &session_id, Some(2));
        assert_eq!(gap_ranges.resolved_session_id.to_string(), session_id);
        assert_eq!(gap_ranges.total_entries, 5);
        assert_eq!(
            gap_ranges.gap_ranges,
            ...
            "--tail <n> must return only the newest n gap ranges, preserving ascending order"
        );
    ...
    #[test]
    fn transcript_gap_ranges_tail_larger_than_total_returns_all_ranges_unchanged() {
        let root = temp_session_root();
        let engine = temp_engine(&root);

        let session_id = seed_triple_gap_transcript(&engine);

        let gap_ranges = transcript_gap_ranges_tail_output(&engine, &session_id, Some(99));
        assert_eq!(gap_ranges.total_entries, 5);
        assert_eq!(
            gap_ranges.gap_ranges,
            vec![
            ...
            "oversized --tail must return every gap range cleanly"
        );
    ...
    #[test]
    fn transcript_gap_ranges_tail_empty_and_contiguous_transcripts_return_empty_cleanly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        for tail in [None, Some(0), Some(5)] {
            let gap_ranges = transcript_gap_ranges_tail_output(&engine, &empty_id, tail);
            assert_eq!(gap_ranges.total_entries, 0);
            assert!(gap_ranges.gap_ranges.is_empty());
        }
        ...
        extend_transcript(&engine, &contiguous_id, &["second prompt", "third prompt"]);
        for tail in [None, Some(0), Some(2), Some(99)] {
            let gap_ranges = transcript_gap_ranges_tail_output(&engine, &contiguous_id, tail);
            assert_eq!(gap_ranges.total_entries, 3);
            assert!(gap_ranges.gap_ranges.is_empty());
        }
    ...
    #[test]
    fn transcript_gap_ranges_tail_latest_selector_resolves_to_newest_persisted_session() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let gap_transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(4),
                    prompt: Prompt::new("newer turn four - gap at 2, 3"),
                },
                TranscriptEntry {
                    turn_index: TurnIndex(8),
                    prompt: Prompt::new("newer turn eight - gap at 5, 6, 7"),
                },
            ],
        };
        engine
        ...
        let gap_ranges = transcript_gap_ranges_tail_output(&engine, "latest", Some(1));
        assert_eq!(gap_ranges.selector, "latest");
        assert_eq!(gap_ranges.resolved_session_id.to_string(), newer);
        assert_eq!(gap_ranges.total_entries, 3);
        assert_eq!(
            ...
            "--tail must surface the newest gap range of the resolved latest session"
        );
    ...
    #[test]
    fn transcript_gap_ranges_tail_label_selector_surfaces_resolved_id_and_does_not_mutate_store() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        engine
            .rename_session(&session_id, "runtime-review")
            .expect("attach label for gap-ranges --tail");

        let gap_ranges =
            transcript_gap_ranges_tail_output(&engine, "label:runtime-review", Some(2));
        assert_eq!(gap_ranges.selector, "label:runtime-review");
        assert_eq!(gap_ranges.resolved_session_id.to_string(), session_id);
        assert_eq!(gap_ranges.total_entries, 5);
        assert_eq!(
        ...
        let reloaded = engine
            .load_session(&session_id)
            .expect("reload after gap-ranges --tail");
        assert_eq!(reloaded.label.as_deref(), Some("runtime-review"));
    ...
    #[test]
    fn transcript_gap_ranges_tail_does_not_mutate_persisted_transcript_or_metadata() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let _ = transcript_gap_ranges_tail_output(&engine, &session_id, Some(0));
        let _ = transcript_gap_ranges_tail_output(&engine, &session_id, Some(1));
        let _ = transcript_gap_ranges_tail_output(&engine, &session_id, Some(99));
    ...
    #[test]
    fn transcript_gap_ranges_invalid_tail_is_rejected_by_clap_parse() {
        use clap::Parser;
    ...
    #[test]
    fn transcript_gap_ranges_limit_and_tail_are_mutually_exclusive_at_parse_time() {
        use clap::Parser;
    ...
    #[test]
    fn transcript_largest_gap_raw_id_contiguous_transcript_reports_null_and_leaves_state_untouched(
    ) {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let output = render_command(
            &engine,
            CliCommand::TranscriptLargestGap {
                selector: id.clone(),
            },
        );
        let largest_gap: SessionTranscriptLargestGap =
            serde_json::from_str(&output).expect("parse transcript-largest-gap output");

        assert_eq!(largest_gap.selector, id);
        assert_eq!(
            largest_gap.resolved_session_id.to_string(),
            largest_gap.selector
        );
        assert_eq!(largest_gap.total_entries, 3);
        assert!(largest_gap.largest_gap.is_none());
        assert_eq!(largest_gap.created_at_ms, before_transcript.created_at_ms);
        assert_eq!(largest_gap.updated_at_ms, before_transcript.updated_at_ms);

        let after_transcript = engine
            .load_transcript(&largest_gap.selector)
            .expect("reload after largest-gap");
        assert_eq!(after_transcript, before_transcript);
        let after_session = engine
            .load_session(&largest_gap.selector)
            .expect("reload session after largest-gap");
        assert_eq!(after_session, before_session);

        let normalized =
            normalize_timestamps(&output.replace(&largest_gap.selector, "<session-id>"));
        assert_eq!(
            normalized,
            readme_output_block("transcript-largest-gap <selector>", "json")
        );
    ...
    #[test]
    fn transcript_largest_gap_latest_selector_targets_most_recently_active_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let largest_gap: SessionTranscriptLargestGap =
            serde_json::from_str(&output).expect("parse transcript-largest-gap latest output");

        assert_eq!(largest_gap.selector, "latest");
        assert_eq!(largest_gap.resolved_session_id.to_string(), newer);
        assert_eq!(largest_gap.total_entries, 2);
        assert!(largest_gap.largest_gap.is_none());
    ...
    #[test]
    fn transcript_largest_gap_label_selector_resolves_to_labeled_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        engine
            .rename_session(&id, "runtime-review")
            .expect("attach label for largest-gap");
        ...
        let largest_gap: SessionTranscriptLargestGap =
            serde_json::from_str(&output).expect("parse transcript-largest-gap label output");

        assert_eq!(largest_gap.selector, "label:runtime-review");
        assert_eq!(largest_gap.resolved_session_id.to_string(), id);
        assert_eq!(largest_gap.total_entries, 2);
        assert!(largest_gap.largest_gap.is_none());
    ...
    #[test]
    fn transcript_largest_gap_single_entry_transcript_reports_null() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let largest_gap = engine
            .largest_gap_session_transcript(&id)
            .expect("single-entry transcript should succeed");
        assert_eq!(largest_gap.total_entries, 1);
        assert!(largest_gap.largest_gap.is_none());
    ...
    #[test]
    fn transcript_largest_gap_empty_transcript_reports_null_cleanly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let largest_gap = engine
            .largest_gap_session_transcript(&session_id)
            .expect("empty transcript should succeed");
        assert_eq!(largest_gap.selector, session_id);
        assert_eq!(
            largest_gap.resolved_session_id.to_string(),
            largest_gap.selector
        );
        assert_eq!(largest_gap.total_entries, 0);
        assert!(largest_gap.largest_gap.is_none());
        assert_eq!(largest_gap.created_at_ms, empty_transcript.created_at_ms);
        assert_eq!(largest_gap.updated_at_ms, empty_transcript.updated_at_ms);
    ...
    #[test]
    fn transcript_largest_gap_single_gap_run_reports_that_run() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let single_gap_transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(5),
                    prompt: Prompt::new("turn five - gap at 2, 3, 4"),
                },
            ],
        };
        engine
        ...
        assert_eq!(
            ...
            Some(SessionTranscriptLargestGapRun {
                start_turn_index: 2,
                end_turn_index: 4,
                missing_count: 3,
            })
    ...
    #[test]
    fn transcript_largest_gap_single_missing_turn_collapses_to_singleton_run() {
        use harness_core::{Prompt, TurnIndex};
        ...
        engine
            ...
            .expect("persist single-missing-turn transcript");

        let largest_gap = engine
            .largest_gap_session_transcript(&session_id)
            .expect("single-missing-turn transcript should succeed");
    ...
    #[test]
    fn transcript_largest_gap_multiple_gap_runs_selects_largest_by_missing_count() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(4),
                    prompt: Prompt::new("turn four - gap at 2, 3 (size 2)"),
                },
                TranscriptEntry {
                    turn_index: TurnIndex(5),
                    prompt: Prompt::new("turn five"),
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(9),
                    prompt: Prompt::new("turn nine - gap at 6, 7, 8 (size 3)"),
                },
            ],
        };
        engine
        ...
        assert_eq!(largest_gap.total_entries, 4);
        assert_eq!(
            largest_gap.largest_gap,
            Some(SessionTranscriptLargestGapRun {
                start_turn_index: 6,
        ...
        assert_eq!(largest_gap.created_at_ms, transcript.created_at_ms);
        assert_eq!(largest_gap.updated_at_ms, transcript.updated_at_ms);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn transcript_largest_gap_tie_selects_earliest_run_by_lowest_start_turn_index() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(4),
                    prompt: Prompt::new("turn four - gap at 2, 3 (first size-2 run)"),
                },
                TranscriptEntry {
                    turn_index: TurnIndex(5),
                    prompt: Prompt::new("turn five"),
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(8),
                    prompt: Prompt::new("turn eight - gap at 6, 7 (second size-2 run)"),
                },
            ],
        };
        engine
            .store
            .save_transcript(&transcript)
            .expect("persist tie transcript");

        let largest_gap = engine
            .largest_gap_session_transcript(&session_id)
            .expect("tie transcript should succeed");
    ...
    #[test]
    fn transcript_largest_gap_unknown_id_and_label_surface_session_not_found() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .largest_gap_session_transcript("00000000-0000-0000-0000-000000000000")
            .expect_err("unknown id should fail");
        assert!(
            err.contains("session not found"),
            "expected session not found for unknown id, got: {err}"
        ...
        let err = engine
            .largest_gap_session_transcript("label:nonexistent")
            .expect_err("unknown label should fail");
        assert!(
            err.contains("session not found"),
            "expected session not found for unknown label, got: {err}"
    ...
    #[test]
    fn transcript_largest_gap_duplicate_label_surfaces_ambiguous_label() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .largest_gap_session_transcript("label:duplicate")
            .expect_err("duplicate label should fail");
        assert!(
            err.contains("ambiguous session label"),
            "expected ambiguous session label error, got: {err}"
    ...
    #[test]
    fn transcript_largest_gap_empty_label_surfaces_malformed_selector() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .largest_gap_session_transcript("label:")
            .expect_err("empty label should fail");
        assert!(
            err.contains("malformed session selector"),
            "expected malformed session selector error, got: {err}"
    ...
    #[test]
    fn transcript_smallest_gap_raw_id_contiguous_transcript_reports_null_and_leaves_state_untouched(
    ) {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let output = render_command(
            &engine,
            CliCommand::TranscriptSmallestGap {
                selector: id.clone(),
            },
        );
        let smallest_gap: SessionTranscriptSmallestGap =
            serde_json::from_str(&output).expect("parse transcript-smallest-gap output");

        assert_eq!(smallest_gap.selector, id);
        assert_eq!(
            smallest_gap.resolved_session_id.to_string(),
            smallest_gap.selector
        );
        assert_eq!(smallest_gap.total_entries, 3);
        assert!(smallest_gap.smallest_gap.is_none());
        assert_eq!(smallest_gap.created_at_ms, before_transcript.created_at_ms);
        assert_eq!(smallest_gap.updated_at_ms, before_transcript.updated_at_ms);

        let after_transcript = engine
            .load_transcript(&smallest_gap.selector)
            .expect("reload after smallest-gap");
        assert_eq!(after_transcript, before_transcript);
        let after_session = engine
            .load_session(&smallest_gap.selector)
            .expect("reload session after smallest-gap");
        assert_eq!(after_session, before_session);

        let normalized =
            normalize_timestamps(&output.replace(&smallest_gap.selector, "<session-id>"));
        assert_eq!(
            normalized,
            readme_output_block("transcript-smallest-gap <selector>", "json")
        );
    ...
    #[test]
    fn transcript_smallest_gap_latest_selector_targets_most_recently_active_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let smallest_gap: SessionTranscriptSmallestGap =
            serde_json::from_str(&output).expect("parse transcript-smallest-gap latest output");

        assert_eq!(smallest_gap.selector, "latest");
        assert_eq!(smallest_gap.resolved_session_id.to_string(), newer);
        assert_eq!(smallest_gap.total_entries, 2);
        assert!(smallest_gap.smallest_gap.is_none());
    ...
    #[test]
    fn transcript_smallest_gap_label_selector_resolves_to_labeled_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        engine
            .rename_session(&id, "runtime-review")
            .expect("attach label for smallest-gap");
        ...
        let smallest_gap: SessionTranscriptSmallestGap =
            serde_json::from_str(&output).expect("parse transcript-smallest-gap label output");

        assert_eq!(smallest_gap.selector, "label:runtime-review");
        assert_eq!(smallest_gap.resolved_session_id.to_string(), id);
        assert_eq!(smallest_gap.total_entries, 2);
        assert!(smallest_gap.smallest_gap.is_none());
    ...
    #[test]
    fn transcript_smallest_gap_single_entry_transcript_reports_null() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let smallest_gap = engine
            .smallest_gap_session_transcript(&id)
            .expect("single-entry transcript should succeed");
        assert_eq!(smallest_gap.total_entries, 1);
        assert!(smallest_gap.smallest_gap.is_none());
    ...
    #[test]
    fn transcript_smallest_gap_empty_transcript_reports_null_cleanly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let smallest_gap = engine
            .smallest_gap_session_transcript(&session_id)
            .expect("empty transcript should succeed");
        assert_eq!(smallest_gap.selector, session_id);
        assert_eq!(
            smallest_gap.resolved_session_id.to_string(),
            smallest_gap.selector
        );
        assert_eq!(smallest_gap.total_entries, 0);
        assert!(smallest_gap.smallest_gap.is_none());
        assert_eq!(smallest_gap.created_at_ms, empty_transcript.created_at_ms);
        assert_eq!(smallest_gap.updated_at_ms, empty_transcript.updated_at_ms);
    ...
    #[test]
    fn transcript_smallest_gap_single_gap_run_reports_that_run() {
        use harness_core::{Prompt, TurnIndex};
        ...
        assert_eq!(
            ...
            Some(SessionTranscriptSmallestGapRun {
                start_turn_index: 2,
                end_turn_index: 4,
                missing_count: 3,
            })
    ...
    #[test]
    fn transcript_smallest_gap_single_missing_turn_collapses_to_singleton_run() {
        use harness_core::{Prompt, TurnIndex};
    ...
    #[test]
    fn transcript_smallest_gap_multiple_gap_runs_selects_smallest_by_missing_count() {
        use harness_core::{Prompt, TurnIndex};
        ...
        assert_eq!(smallest_gap.total_entries, 4);
        assert_eq!(
            smallest_gap.smallest_gap,
            Some(SessionTranscriptSmallestGapRun {
                start_turn_index: 2,
        ...
        assert_eq!(smallest_gap.created_at_ms, transcript.created_at_ms);
        assert_eq!(smallest_gap.updated_at_ms, transcript.updated_at_ms);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn transcript_smallest_gap_tie_selects_earliest_run_by_lowest_start_turn_index() {
        use harness_core::{Prompt, TurnIndex};
    ...
    #[test]
    fn transcript_smallest_gap_unknown_id_and_label_surface_session_not_found() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .smallest_gap_session_transcript("00000000-0000-0000-0000-000000000000")
            .expect_err("unknown id should fail");
        assert!(
            err.contains("session not found"),
            "expected session not found for unknown id, got: {err}"
        ...
        let err = engine
            .smallest_gap_session_transcript("label:nonexistent")
            .expect_err("unknown label should fail");
        assert!(
            err.contains("session not found"),
            "expected session not found for unknown label, got: {err}"
    ...
    #[test]
    fn transcript_smallest_gap_duplicate_label_surfaces_ambiguous_label() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .smallest_gap_session_transcript("label:duplicate")
            .expect_err("duplicate label should fail");
        assert!(
            err.contains("ambiguous session label"),
            "expected ambiguous session label error, got: {err}"
    ...
    #[test]
    fn transcript_smallest_gap_empty_label_surfaces_malformed_selector() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .smallest_gap_session_transcript("label:")
            .expect_err("empty label should fail");
        assert!(
            err.contains("malformed session selector"),
            "expected malformed session selector error, got: {err}"
    ...
    #[test]
    fn transcript_gap_count_raw_id_contiguous_transcript_reports_zero_and_leaves_state_untouched() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let output = render_command(
            &engine,
            CliCommand::TranscriptGapCount {
                selector: id.clone(),
            },
        );
        let gap_count: SessionTranscriptGapCount =
            serde_json::from_str(&output).expect("parse transcript-gap-count output");

        assert_eq!(gap_count.selector, id);
        assert_eq!(gap_count.resolved_session_id.to_string(), gap_count.selector);
        assert_eq!(gap_count.total_entries, 3);
        assert_eq!(gap_count.gap_count, 0);
        assert_eq!(gap_count.created_at_ms, before_transcript.created_at_ms);
        assert_eq!(gap_count.updated_at_ms, before_transcript.updated_at_ms);

        let after_transcript = engine
            .load_transcript(&gap_count.selector)
            .expect("reload after gap-count");
        assert_eq!(after_transcript, before_transcript);
        let after_session = engine
            .load_session(&gap_count.selector)
            .expect("reload session after gap-count");
        assert_eq!(after_session, before_session);

        let normalized = normalize_timestamps(&output.replace(&gap_count.selector, "<session-id>"));
        assert_eq!(
            normalized,
            readme_output_block("transcript-gap-count <selector>", "json")
        );
    ...
    #[test]
    fn transcript_gap_count_latest_selector_targets_most_recently_active_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let gap_count: SessionTranscriptGapCount =
            serde_json::from_str(&output).expect("parse transcript-gap-count latest output");

        assert_eq!(gap_count.selector, "latest");
        assert_eq!(gap_count.resolved_session_id.to_string(), newer);
        assert_eq!(gap_count.total_entries, 2);
        assert_eq!(gap_count.gap_count, 0);
    ...
    #[test]
    fn transcript_gap_count_label_selector_resolves_to_labeled_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        engine
            .rename_session(&id, "runtime-review")
            .expect("attach label for gap-count");
        ...
        let gap_count: SessionTranscriptGapCount =
            serde_json::from_str(&output).expect("parse transcript-gap-count label output");

        assert_eq!(gap_count.selector, "label:runtime-review");
        assert_eq!(gap_count.resolved_session_id.to_string(), id);
        assert_eq!(gap_count.total_entries, 2);
        assert_eq!(gap_count.gap_count, 0);

        let reloaded = engine.load_session(&id).expect("reload after gap-count");
        assert_eq!(reloaded.label.as_deref(), Some("runtime-review"));
    ...
    #[test]
    fn transcript_gap_count_single_entry_transcript_reports_zero() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let gap_count = engine
            .gap_count_session_transcript(&id)
            .expect("single-entry transcript should succeed");
        assert_eq!(gap_count.total_entries, 1);
        assert_eq!(gap_count.gap_count, 0);
    ...
    #[test]
    fn transcript_gap_count_empty_transcript_reports_zero_cleanly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let gap_count = engine
            .gap_count_session_transcript(&session_id)
            .expect("empty transcript should succeed");
        assert_eq!(gap_count.selector, session_id);
        assert_eq!(gap_count.resolved_session_id.to_string(), gap_count.selector);
        assert_eq!(gap_count.total_entries, 0);
        assert_eq!(gap_count.gap_count, 0);
        assert_eq!(gap_count.created_at_ms, empty_transcript.created_at_ms);
        assert_eq!(gap_count.updated_at_ms, empty_transcript.updated_at_ms);
    ...
    #[test]
    fn transcript_gap_count_single_gap_run_reports_one() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let single_gap_transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(5),
                    prompt: Prompt::new("turn five - adjacent missing 2,3,4 collapse to one run"),
                },
            ],
        };
        engine
        ...
        assert_eq!(gap_count.total_entries, 2);
        assert_eq!(gap_count.gap_count, 1);
    ...
    #[test]
    fn transcript_gap_count_multiple_disjoint_gap_runs_reports_count() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(4),
                    prompt: Prompt::new("turn four - gap at 2,3 (run 1)"),
                },
                TranscriptEntry {
                    turn_index: TurnIndex(5),
                    prompt: Prompt::new("turn five"),
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(7),
                    prompt: Prompt::new("turn seven - gap at 6 (run 2)"),
                },
                TranscriptEntry {
                    turn_index: TurnIndex(11),
                    prompt: Prompt::new("turn eleven - gap at 8,9,10 (run 3)"),
                },
            ],
        };
        engine
        ...
        assert_eq!(gap_count.total_entries, 5);
        assert_eq!(gap_count.gap_count, 3);
        assert_eq!(gap_count.created_at_ms, transcript.created_at_ms);
        assert_eq!(gap_count.updated_at_ms, transcript.updated_at_ms);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn transcript_gap_count_unknown_id_and_label_surface_session_not_found() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .gap_count_session_transcript("00000000-0000-0000-0000-000000000000")
            .expect_err("unknown id should fail");
        assert!(
            err.contains("session not found"),
            "expected session not found for unknown id, got: {err}"
        ...
        let err = engine
            .gap_count_session_transcript("label:nonexistent")
            .expect_err("unknown label should fail");
        assert!(
            err.contains("session not found"),
            "expected session not found for unknown label, got: {err}"
    ...
    #[test]
    fn transcript_gap_count_duplicate_label_surfaces_ambiguous_label() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .gap_count_session_transcript("label:duplicate")
            .expect_err("duplicate label should fail");
        assert!(
            err.contains("ambiguous session label"),
            "expected ambiguous session label error, got: {err}"
    ...
    #[test]
    fn transcript_gap_count_empty_label_surfaces_malformed_selector() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .gap_count_session_transcript("label:")
            .expect_err("empty label should fail");
        assert!(
            err.contains("malformed session selector"),
            "expected malformed session selector error, got: {err}"
    ...
    #[test]
    fn transcript_missing_turn_count_raw_id_contiguous_transcript_reports_zero_and_leaves_state_untouched(
    ) {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let output = render_command(
            &engine,
            CliCommand::TranscriptMissingTurnCount {
                selector: id.clone(),
            },
        );
        let missing_turn_count: SessionTranscriptMissingTurnCount =
            serde_json::from_str(&output).expect("parse transcript-missing-turn-count output");

        assert_eq!(missing_turn_count.selector, id);
        assert_eq!(
            missing_turn_count.resolved_session_id.to_string(),
            missing_turn_count.selector
        );
        assert_eq!(missing_turn_count.total_entries, 3);
        assert_eq!(missing_turn_count.missing_turn_count, 0);
        assert_eq!(
            missing_turn_count.created_at_ms,
            before_transcript.created_at_ms
        );
        assert_eq!(
            missing_turn_count.updated_at_ms,
            before_transcript.updated_at_ms
        );

        let after_transcript = engine
            .load_transcript(&missing_turn_count.selector)
            .expect("reload after missing-turn-count");
        assert_eq!(after_transcript, before_transcript);
        let after_session = engine
            .load_session(&missing_turn_count.selector)
            .expect("reload session after missing-turn-count");
        assert_eq!(after_session, before_session);

        let normalized =
            normalize_timestamps(&output.replace(&missing_turn_count.selector, "<session-id>"));
        assert_eq!(
            normalized,
            readme_output_block("transcript-missing-turn-count <selector>", "json")
        );
    ...
    #[test]
    fn transcript_missing_turn_count_latest_selector_targets_most_recently_active_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let missing_turn_count: SessionTranscriptMissingTurnCount = serde_json::from_str(&output)
            .expect("parse transcript-missing-turn-count latest output");

        assert_eq!(missing_turn_count.selector, "latest");
        assert_eq!(missing_turn_count.resolved_session_id.to_string(), newer);
        assert_eq!(missing_turn_count.total_entries, 2);
        assert_eq!(missing_turn_count.missing_turn_count, 0);
    ...
    #[test]
    fn transcript_missing_turn_count_label_selector_resolves_to_labeled_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        engine
            .rename_session(&id, "runtime-review")
            .expect("attach label for missing-turn-count");
        ...
        let missing_turn_count: SessionTranscriptMissingTurnCount = serde_json::from_str(&output)
            .expect("parse transcript-missing-turn-count label output");

        assert_eq!(missing_turn_count.selector, "label:runtime-review");
        assert_eq!(missing_turn_count.resolved_session_id.to_string(), id);
        assert_eq!(missing_turn_count.total_entries, 2);
        assert_eq!(missing_turn_count.missing_turn_count, 0);
    ...
    #[test]
    fn transcript_missing_turn_count_single_entry_transcript_reports_zero() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let missing_turn_count = engine
            .missing_turn_count_session_transcript(&id)
            .expect("single-entry transcript should succeed");
        assert_eq!(missing_turn_count.total_entries, 1);
        assert_eq!(missing_turn_count.missing_turn_count, 0);
    ...
    #[test]
    fn transcript_missing_turn_count_empty_transcript_reports_zero_cleanly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let missing_turn_count = engine
            .missing_turn_count_session_transcript(&session_id)
            .expect("empty transcript should succeed");
        assert_eq!(missing_turn_count.selector, session_id);
        assert_eq!(
            missing_turn_count.resolved_session_id.to_string(),
            missing_turn_count.selector
        );
        assert_eq!(missing_turn_count.total_entries, 0);
        assert_eq!(missing_turn_count.missing_turn_count, 0);
        assert_eq!(
            missing_turn_count.created_at_ms,
            empty_transcript.created_at_ms
        );
        assert_eq!(
            missing_turn_count.updated_at_ms,
            empty_transcript.updated_at_ms
        );
    ...
    #[test]
    fn transcript_missing_turn_count_one_internal_gap_reports_run_size() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let single_gap_transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(5),
                    prompt: Prompt::new("turn five - missing 2,3,4 = 3 individual missing turns"),
                },
            ],
        };
        engine
        ...
        assert_eq!(missing_turn_count.total_entries, 2);
        assert_eq!(missing_turn_count.missing_turn_count, 3);
    ...
    #[test]
    fn transcript_missing_turn_count_multiple_internal_gaps_sums_individual_missing_turns() {
        use harness_core::{Prompt, TurnIndex};
        ...
        let transcript = TranscriptRecord {
            ...
            entries: vec![
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(4),
                    prompt: Prompt::new("turn four - missing 2,3 (run 1, 2 turns)"),
                },
                TranscriptEntry {
                    turn_index: TurnIndex(5),
                    prompt: Prompt::new("turn five"),
                ...
                TranscriptEntry {
                    turn_index: TurnIndex(7),
                    prompt: Prompt::new("turn seven - missing 6 (run 2, 1 turn)"),
                },
                TranscriptEntry {
                    turn_index: TurnIndex(11),
                    prompt: Prompt::new("turn eleven - missing 8,9,10 (run 3, 3 turns)"),
                },
            ],
        };
        engine
        ...
        assert_eq!(missing_turn_count.total_entries, 5);
        assert_eq!(missing_turn_count.missing_turn_count, 6);
        assert_eq!(missing_turn_count.created_at_ms, transcript.created_at_ms);
        assert_eq!(missing_turn_count.updated_at_ms, transcript.updated_at_ms);

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn transcript_missing_turn_count_unknown_id_and_label_surface_session_not_found() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .missing_turn_count_session_transcript("00000000-0000-0000-0000-000000000000")
            .expect_err("unknown id should fail");
        assert!(
            err.contains("session not found"),
            "expected session not found for unknown id, got: {err}"
        ...
        let err = engine
            .missing_turn_count_session_transcript("label:nonexistent")
            .expect_err("unknown label should fail");
        assert!(
            err.contains("session not found"),
            "expected session not found for unknown label, got: {err}"
    ...
    #[test]
    fn transcript_missing_turn_count_duplicate_label_surfaces_ambiguous_label() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .missing_turn_count_session_transcript("label:duplicate")
            .expect_err("duplicate label should fail");
        assert!(
            err.contains("ambiguous session label"),
            "expected ambiguous session label error, got: {err}"
    ...
    #[test]
    fn transcript_missing_turn_count_empty_label_surfaces_malformed_selector() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let err = engine
            .missing_turn_count_session_transcript("label:")
            .expect_err("empty label should fail");
        assert!(
            err.contains("malformed session selector"),
            "expected malformed session selector error, got: {err}"
    ...
    fn transcript_show_tail_record(
        engine: &RuntimeEngine,
        selector: &str,
        tail: Option<usize>,
    ) -> TranscriptRecord {
        ...
        serde_json::from_str(&output).expect("parse transcript-show tail output")
    }
    ...
    #[test]
    fn transcript_show_omitted_tail_preserves_unlimited_behavior_exactly() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
    ...
    #[test]
    fn transcript_show_tail_zero_returns_empty_entries_without_mutating_store() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let record = transcript_show_tail_record(&engine, &id, Some(0));
        assert_eq!(record.session_id.to_string(), id);
        assert!(
            record.entries.is_empty(),
            "--tail 0 must return an empty entries array"
        );
    ...
    #[test]
    fn transcript_show_tail_returns_newest_n_entries_in_append_order() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let record = transcript_show_tail_record(&engine, &id, Some(2));
        assert_eq!(record.session_id.to_string(), id);
        let turns: Vec<(usize, String)> = record
            .entries
            .iter()
        ...
        assert_eq!(
            ...
            "--tail <n> must return only the newest n entries, preserving canonical append order inside the slice"
        );
    ...
    #[test]
    fn transcript_show_tail_larger_than_total_returns_all_entries_unchanged() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let record = transcript_show_tail_record(&engine, &id, Some(99));
        assert_eq!(record.session_id.to_string(), id);
        let turns: Vec<(usize, String)> = record
            .entries
            .iter()
        ...
        assert_eq!(
            ...
            "oversized --tail must return every persisted entry cleanly"
        );
    ...
    #[test]
    fn transcript_show_tail_latest_selector_resolves_to_newest_persisted_session() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let record = transcript_show_tail_record(&engine, "latest", Some(1));
        assert_eq!(
            record.session_id.to_string(),
            newer,
            "latest must resolve to the most recent persisted session under --tail"
        ...
        assert_eq!(record.entries.len(), 1);
        assert_eq!(record.entries[0].turn_index.0, 2);
        assert_eq!(
            record.entries[0].prompt.0, "newer third",
            "--tail 1 must surface the newest persisted entry"
        );
    ...
    #[test]
    fn transcript_show_tail_label_selector_surfaces_resolved_id_and_does_not_mutate_store() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        engine
            .rename_session(&id, "runtime-review")
            .expect("attach label for transcript-show --tail");
        ...
        let record: TranscriptRecord =
            serde_json::from_str(&output).expect("parse labeled transcript-show tail");
        assert_eq!(record.session_id.to_string(), id);
        assert!(
        ...
        assert_eq!(
            ...
            vec![
                (1, "labeled second".to_string()),
                (2, "labeled third".to_string()),
            ],
            "--tail <n> must surface the newest n entries for a label selector"
        );

        let reloaded_session = engine
            .load_session(&id)
            .expect("reload after transcript-show --tail");
        assert_eq!(reloaded_session.label.as_deref(), Some("runtime-review"));
        let reloaded_transcript = engine
            .load_transcript(&id)
            .expect("reload transcript after transcript-show --tail");
        assert_eq!(
            reloaded_transcript.entries.len(),
            3,
            "persisted transcript entries must not be mutated by --tail"
        );
    ...
    #[test]
    fn transcript_show_tail_raw_id_selector_surfaces_resolved_id() {
        let root = temp_session_root();
        let engine = temp_engine(&root);
        ...
        let record = transcript_show_tail_record(&engine, &id, Some(1));
        assert_eq!(
            record.session_id.to_string(),
            id,
        ...
        assert_eq!(record.entries.len(), 1);
        assert_eq!(record.entries[0].turn_index.0, 2);
        assert_eq!(record.entries[0].prompt.0, "third prompt");

        fs::remove_dir_all(&root).expect("remove temp cli test directory");
    }

    #[test]
    fn transcript_show_invalid_tail_is_rejected_by_clap_parse() {
        use clap::Parser;
    ...
    #[test]
    fn transcript_show_limit_and_tail_are_mutually_exclusive_at_parse_time() {
        use clap::Parser;
