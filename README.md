...
Pass `--limit <n>` to cap the `messages` array to at most the first `n` messages in the existing canonical append order. The outer JSON shape is preserved — `--limit` only truncates `messages`; no wrapper object is introduced. `--limit 0` returns `messages: []` cleanly, a `--limit` larger than the persisted message count returns every message cleanly, and omitting `--limit` preserves the unlimited behavior below exactly. Negative and non-numeric `--limit` values fail cleanly at parse time. No persisted session state, transcript entry, label, pinned flag, id, path, or ordering metadata is mutated.
...
Pass `--limit <n>` to cap the `matches` array to at most the first `n` entries in the existing source transcript `turn_index` ordering. The outer JSON shape is preserved — `--limit` only truncates `matches` and recomputes `match_count` so `match_count == matches.len()` still holds; no wrapper object is introduced. Query and selector semantics are preserved — an empty query and a no-match query still return `match_count: 0` with an empty `matches` array regardless of `--limit`. `--limit 0` returns `match_count: 0` with an empty `matches` array cleanly, a `--limit` larger than the match count returns every match unchanged, and omitting `--limit` preserves the unlimited behavior above exactly. Negative and non-numeric `--limit` values fail cleanly at parse time.
