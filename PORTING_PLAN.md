...
- [x] `session-pin <id>` / `session-unpin <id>` (and `latest` / `label:<name>` variants) — toggle a deterministic `pinned` flag on persisted session metadata while preserving the existing `session_id`, leaving transcript entries and ordering untouched, and not bumping `updated_at_ms` so newest-first ordering stays activity-based; the same shared selector machinery accepts raw ids, `latest`, and `label:<name>`; output is a deterministic `{ pinned_session_id, pinned }` / `{ unpinned_session_id, pinned }` JSON shape; unknown sessions/selectors surface as `SessionNotFound`, already-pinned/unpinned states surface as `SessionAlreadyPinned` / `SessionAlreadyUnpinned` rather than silent no-ops; the persisted JSON serializes the `pinned` field only when the session is actually pinned so older sessions stay byte-compatible; pinned state also surfaces through `sessions`, `session-show`, `session-export`, `session-compare`, and `session-labels`, and `session-prune --keep <count>` excludes pinned sessions from pruning and surfaces them via `pinned_preserved_count` / `pinned_preserved`
